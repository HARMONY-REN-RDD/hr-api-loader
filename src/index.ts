import path from "path";
import fs from "fs";
import { MethodNotFoundError, VersionNotSupportedError } from "./error";

type MethodMap = {
  [key: string]: (...args: any[]) => any;
};

type VersionedMethodMap = {
  [version: string]: MethodMap;
};

class ApiLoader {
  api_path: string;
  methods: VersionedMethodMap = {};

  constructor() {
    const api_path_arg = process.argv[1];
    if (!api_path_arg) throw new Error("Script path (argv[1]) is undefined");
    this.api_path = path.join(path.dirname(api_path_arg), 'api');
    if (!fs.existsSync(this.api_path)) throw new Error(`API folder does not exist at path: ${this.api_path}`);
  }

  async init() {
    const versions = fs.readdirSync(this.api_path);
    for (const version of versions) {
      this.methods[version] = {};
      const version_path = path.join(this.api_path, version);
      await this.load_methods_recursively(version_path, this.methods[version], []);
    }
  }

  private async load_methods_recursively(dir_path: string, method_map: MethodMap, path_parts: string[]) {
    for (const entry of fs.readdirSync(dir_path)) {
      const full_path = path.join(dir_path, entry);
      const stat = fs.statSync(full_path);
      if (stat.isDirectory()) {
        await this.load_methods_recursively(full_path, method_map, [...path_parts, entry]);
      } else if (stat.isFile() && entry.endsWith('.js')) {
        const filename = path.parse(entry).name;
        const method_key = [...path_parts, filename].join('-');
        try {
          const mod = require(full_path);
          const fn = mod.default || mod;
          method_map[method_key] = fn;
        } catch (e) {
          console.error(`Failed to load method ${method_key} from ${full_path}`, e);
        }
      }
    }
  }

  call(version: string, method: string, ...args: any[]) {
    if (!this.methods[version]) throw new VersionNotSupportedError(`Version ${version} is not supported`);
    if (!this.methods[version][method]) throw new MethodNotFoundError(`Method ${method} not found in version ${version}`);
    return this.methods[version][method](...args);
  }
}

export default ApiLoader;

async function main() {
  const loader = new ApiLoader();
  await loader.init();
  console.log(loader.methods);
}

main();
